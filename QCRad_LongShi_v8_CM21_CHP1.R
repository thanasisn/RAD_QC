...
for (YY in yearSTA:yearEND) {
    ...
    if (all(DO_TEST_01, DO_TEST_02)) {
        if (any(!is.na(DATA_year$wattDIR))) {
            ...
            points(DATA_year$SZA, DATA_year$TSIextEARTH_comb,  cex = .1,  col = alpha("red", 0.05))
            ## 2. Extremely rare limits max (4)
            points(DATA_year$SZA, Direct_max_extremely_rare,   cex = .1,  col = alpha("blue",0.05))
    ...
    if (DO_TEST_04 & !all(is.na(DATA_year$wattDIR))) {
        ...
        if (any(!is.na(DATA_year$wattDIR))) {
            ...
            ## 4. First climatological limit (17)
            points(DATA_year$Azimuth, first_level_D,   cex = .1,  col = alpha("blue",0.05))
